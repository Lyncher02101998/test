...
recipient_emails = ['bcc_bot@mail.ru'] #'itsupport@bcc.kz'почта на отправку заявки
...
def save_request_to_db(terminal_id, failure_type, callback):
    ...
    if existing_entries_count == 0:
        ...
        bot.send_message(callback.message.chat.id, f'Заяка на идентификатор терминала {terminal_id} отправлена!')
